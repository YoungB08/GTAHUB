...
// Đăng ký nhận danh sách nhân vật thực tế từ Server Pawn (Đăng ký sớm)
if (window.cef) {
    cef.on("GTAHUB:CharacterList", (data) => {
        try {
            console.log("[CEF] Nhận được GTAHUB:CharacterList:", data);
...
function init() {
    emitCEF("GTAHUB:CharSelectReady");
    renderCharList();
    updatePreview();
...
function renderCharList() {
    ...
    if (!characters || characters.length === 0) {
        ...
        emptyNotice.className = "p-5 text-center text-slate-400 text-xs border border-dashed border-white/10 rounded-2xl bg-slate-950/30";
        emptyNotice.innerHTML = `
            <p class="font-bold text-white mb-1">Chưa có nhân vật nào</p>
            <p class="text-[11px] text-slate-500">Hãy bấm nút "Tạo Nhân Vật Mới" bên dưới để bắt đầu nhập vai!</p>
        `;
        charListContainer.appendChild(emptyNotice);
    } else {
        characters.forEach((char, index) => {
            ...
            card.className = `p-4 border rounded-2xl cursor-pointer relative transition-all duration-200 hover:scale-[1.02] flex justify-between items-center ${
                isSelected 
                ? "bg-gradient-to-r from-red-950/70 to-slate-900/90 border-red-500 shadow-[0_0_20px_rgba(239,68,68,0.25)]" 
            ...
            card.innerHTML = `
                <div class="flex-1">
                    ...
                    <p class="text-[10px] ${isSelected ? 'text-red-400' : 'text-slate-400'} font-medium mt-0.5">Cấp độ: ${char.level} • Tiền: $${(char.cash || 0).toLocaleString()}</p>
                    <div class="mt-2 text-[9px] text-slate-500 flex gap-3">
                        <span>SĐT: ${char.phone || "N/A"}</span>
                    </div>
                </div>
    ...
    createCard.className = "p-4 bg-slate-950/40 border border-dashed border-white/15 hover:border-red-500/60 rounded-2xl flex items-center justify-center gap-2 text-slate-400 hover:text-red-400 transition-all cursor-pointer";
...
function updatePreview() {
    const char = (characters && characters.length > 0) ? characters[selectedIndex] : null;
    if (!char) {
        if (previewName) previewName.innerText = "Chưa chọn nhân vật";
        if (previewFaction) previewFaction.innerText = "Tổ chức: -";
        if (previewImage) previewImage.src = "http://gtahub.kntech.co/skins/0.png";
        if (statLevel) statLevel.innerText = "-";
        if (statCash) statCash.innerText = "$0";
        if (statBank) statBank.innerText = "$0";
        if (statPhone) statPhone.innerText = "-";
        if (statVehicles) statVehicles.innerHTML = `<span class="text-slate-500 italic block">Không có</span>`;
        if (previewSkinId) previewSkinId.innerText = "SKIN #0";
        if (enterCityBtn) enterCityBtn.classList.add("opacity-50", "pointer-events-none");
        return;
    }

    if (enterCityBtn) enterCityBtn.classList.remove("opacity-50", "pointer-events-none");
    if (previewName) previewName.innerText = char.name;
    if (previewFaction) previewFaction.innerText = `Tổ chức: ${char.faction || "Dân Thường"}`;
    if (previewImage) previewImage.src = `http://gtahub.kntech.co/skins/${char.skin || 0}.png`;
    if (statLevel) statLevel.innerText = char.level || 1;
    if (statCash) statCash.innerText = `$${(char.cash || 0).toLocaleString()}`;
    if (statBank) statBank.innerText = `$${(char.bank || 0).toLocaleString()}`;
    if (statPhone) statPhone.innerText = char.phone || "N/A";
    ...
    if (previewSkinId) previewSkinId.innerText = `SKIN #${char.skin || 0}`;
}
